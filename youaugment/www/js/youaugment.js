...
function addArObjects(tIndex) {
    ...
    for (i = 0; i < videosArray.length; i++) {
        if (videosArray[i] != '') addVideo(videosIdArray[i], videosArray[i], false);
    }
...
function addVideo(id, videofile) {
    var positionX = 0.1, positionY = 0.1, positionZ = 0.1;
    ...
    videoMat.diffuseTexture = new BABYLON.VideoTexture("video", ["data/" + videofile], scene, false);
    videoMat.backFaceCulling = false;
    //Applying materials
    videoPlane.material = videoMat;
    videoPlane.material.specularColor = new BABYLON.Color3(0, 0, 0);
...
function addImage(id, imageFile, clickaction) {
    ...
    imageMat.diffuseTexture = new BABYLON.Texture("data/" + imageFile, scene);
